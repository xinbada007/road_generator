...
NurbsCurve::NurbsCurve():
	_method(1), _ctrlPts(0), _knots(0), _weights(0), _degree(3), _numPath(20),_updated(false)
{
}
...
NurbsCurve::~NurbsCurve()
{
	std::cout << "Deconstruct NurbsCurve" << std::endl;
}
...
void NurbsCurve::updateImplementation()
{
	...
	else
	{
		...
		if ( numKnots<_degree+numCtrl+1 )
		{
			osg::notify(osg::WARN) << "osgModeling: No enough knots to create a NURBS curve, need " << _degree+numCtrl+1 <<
				" but only " << numKnots << " found. Add values behind.\n";
		}
	...
	if ( !_numPath )
	{
		osg::notify(osg::WARN) << "osgModeling: Invalid parameters to create a NURBS curve.\n";
		return;
	}
